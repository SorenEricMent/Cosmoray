...
"$$ |  $$ |$$   ____|$$ |      $$ |      $$   ____|$$  __$$ |$$ |      $$  __$$ |$$ |  $$ | \n"+
"$$ |  $$ |\\$$$$$$$\\ \\$$$$$$$\\ $$ |      \\$$$$$$$\\ \\$$$$$$$ |$$ |      \\$$$$$$$ |\\$$$$$$$ | \n"+
"\\__|  \\__| \\_______| \\_______|\\__|       \\_______| \\_______|\\__|       \\_______| \\____$$ | \n"+
"                                                                                $$\\   $$ | \n"+
"                                                                                \\$$$$$$  | \n"+
"                                                                                 \\______/ \n" 
,"color:#CCCC00");
	__RecrearayOuterShell();//进入Launcher
});
...
async function __RecrearayOuterShell(){
	...
	preloadProgress = $('<div>', {//插入加载进度条
		id : 'recrearay-loading',
		class : 'mdui-progress'
	});
	preloadProgress.appendTo('body');
	document.getElementById('recrearay-loading').innerHTML = '<div class="mdui-progress-indeterminate"></div>';
	...
	_ConsoleOutput(origin,"Recrearay launched.");
	_ConsoleOutput(origin,"Recrearay, By WinslowEric, for TBLJ");
	MDUIDomLoad();
	if(VM("ENVI_CHECK") == 1){//Cosmoray完成环境检查，开始运行
		var ServerSideVersion = await VersionCheck();
		if((ServerSideVersion - __V__.sv) > 2){
			mdui.alert("请更新你的Recrearay","错误");
			$("#recrearay-loading").fadeOut();
		}else{
			var currentEnvi = websiteCheck();//检查目前处在什么网站
			var isRecrearayEnabled = await isServerAllowed();
			if(isRecrearayEnabled == 1){
				...
				if(currentEnvi == "TIEBA"){//进入贴吧分支
					...
					if(TBPageData.user.is_login){
						...
						if(accountStatus.status == 2){
							...
							$("#recrearay-loading").fadeOut();
							setTimeout("mdui.prompt('激活Recrearay',function (value) {Activate(value);});",1000);
						}
					}else{
...
function _RecrearayGUI(gui){
	const origin = "GUI";
	if(gui == "TBMAIN"){//贴吧主界面GUI
		isAdminMode1 = localStorage.getItem("ibmode");
		isAdminMode2 = localStorage.getItem("sbmode");
		GUIStatus = localStorage.getItem("CMR-guiShow");
		GUIMode = localStorage.getItem("CMR-guiModetb");
		if(isAdminMode1){
			s_ib_Gui = $('<div>', {
				id : 'recrearay-single-ib',
				class : 'mdui-shadow-2 mdui-hoverable mdui-typo'
			});
			s_ib_Gui.appendTo('body');
			document.getElementById('recrearay-single-ib').innerHTML = 
			'<span class="cosmoray-title">Recrearay I</span><br/><span class="cosmoray-subt">By WinslowEric.cn<br/>['+'IV'+__V__.iv+',KSV'+__V__.ksv+',DV'+__V__.dv+']</span><br/>';
		}else if(isAdminMode2){
			s_sb_Gui = $('<div>', {
				id : 'recrearay-single-ib',
				class : 'mdui-shadow-2 mdui-hoverable mdui-typo'
			});
			s_sb_Gui.appendTo('body');
			document.getElementById('recrearay-single-sb').innerHTML = 
			'<span class="cosmoray-title">Recrearay I</span><br/><span class="cosmoray-subt">By WinslowEric.cn<br/>['+'IV'+__V__.iv+',KSV'+__V__.ksv+',DV'+__V__.dv+']</span><br/>';
		}else{
			...
			document.getElementById('cosmoray-main-gui-tb').innerHTML = 
				...
				'<button id="cmrabout" class="mdui-btn mdui-btn-raised mdui-btn-block mdui-color-grey-200">关于Recrearay</button><br/>';
			document.getElementById('cosmoray-st').innerHTML = 
				'<span class="cosmoray-title">Recrearay I</span><span class="cosmoray-subt">By WinslowEric.cn<br/>['+'IV'+__V__.iv+',KSV'+__V__.ksv+',DV'+__V__.dv+']</span><ul class="mdui-list mdui-color-theme-900">'+
			...
			document.getElementById('cosmoray-bw').innerHTML = 
				'<span class="cosmoray-title">Recrearay I</span><br><span class="cosmoray-subt">By WinslowEric.cn<br/>['+'IV'+__V__.iv+',KSV'+__V__.ksv+',DV'+__V__.dv+']</span><br/>'+
				'<button id="bw-setkw" class="mdui-btn mdui-btn-raised mdui-btn-block mdui-color-grey-300 cmrcomponent">设置删帖关键词</button><br/>'+
				'<button id="bw-delstart" class="mdui-btn mdui-btn-raised mdui-btn-block mdui-color-grey-300 cmrcomponent">启动删帖机</button><br/>'+
				'<button id="bw-ibstart" class="mdui-btn mdui-btn-raised mdui-btn-block mdui-color-grey-300 cmrcomponent">启动批量拉黑名单机</button><br/>'+
				'<button id="bw-sbstart" class="mdui-btn mdui-btn-raised mdui-btn-block mdui-color-grey-300 cmrcomponent">启动批量解黑名单机</button><br/>'+
			...
			document.getElementById('bw-ibstart').onclick = function(){
				intoBlacklist();
			};
			document.getElementById('bw-sbstart').onclick = function(){
				solveBlacklist();
			};
			...
			document.getElementById('cmrabout').onclick = function(){
				mdui.alert('Recrearay是WinslowEric.CN为贴吧联军编写的贴吧作战工具。<br/>使用的第三方代码:jQuery,MDUI<br/>特别鸣谢:GSGCDSUN','关于Recrearay');
			};
...
async function getHarass(){
	var harassData = await $.get("");
	return harassData;
}
//获取人名
...
async function wf_stop(){
	_snackBar("挖坟模块已经停止");
	clearInterval(wf_mainLoop);
}

function intoBlacklist(){
	mdui.alert('这个工具将会启动自动拉黑机器，按确定以后，机器将会创建一个新的界面并在里面自动执行拉黑，期间请不要关闭界面！停止拉黑机器请在新打开的网页中点击“停止拉黑机”',
		function (value) {
			window.open("http://tieba.baidu.com/bawu2/platform/listMember?word="+TBPageData.forum.name);
			localStorage.setItem("ibmode", "true");
		});	
}

function solveBlacklist(){
	mdui.alert('这个工具将会启动自动解黑机器，按确定以后，机器将会创建一个新的界面并在里面自动执行解除黑名单，期间请不要关闭界面！停止拉黑机器请在新打开的网页中点击“停止解黑机”',
		function (value) {
			window.open("http://tieba.baidu.com/bawu2/platform/listMember?word="+TBPageData.forum.name);
			localStorage.setItem("sbmode", "true");
		});	
}
//Cosmoray初始化（网站数据）
function _CosmorayInit(site){
	if(site == "TB"){
		...
		dataBridge.innerHTML = 
		"if(JSON.stringify == undefined){document.body.setAttribute('exchange_PageData',"+
		"'{\"user\":{\"name\":\"'+PageData.user.name+'\",\"open_uid\":\"'+PageData.user.open_uid+'\",\"id\":\"'+PageData.user.id+'\"},\"forum\":{\"name\":\"'+PageData.forum.forum_name+'\"}}');"+
		"}else{document.body.setAttribute('exchange_PageData',JSON.stringify(PageData));}";
		document.head.appendChild(dataBridge);
		document.head.removeChild(dataBridge);
		TBPageData = document.body.getAttribute('exchange_PageData');
		TBPageData = JSON.parse(TBPageData);
		document.body.removeAttribute('exchange_PageData');
		return TBPageData;
	}
}
//贴吧UI优化
...
async function isServerAllowed(){
	_RequestData = await $.ajax({
		type : 'GET',
		url : 'https://api.winsloweric.cn/recrearay/cert/allow.php',
		async : true,
		success : function(callback) {
			ALSTATUS = callback;
		}
...
async function testActivate(tiebaID){
	_RequestData = await $.ajax({
		type : 'GET',
		url : 'https://api.winsloweric.cn/recrearay/cert/query.php?uid='+tiebaID,
		async : true,
		success : function(callback) {
			ACSTATUS = callback;
		}
...
async function VersionCheck(){
	var_RequestData = await $.ajax({
		type : 'GET',
		url : 'https://api.winsloweric.cn/recrearay/cert/version.php',
		async : true,
		success : function(callback) {
...
async function getGlobalBlackList(){
	var _RequestData = await $.ajax({
		type : 'GET',
		url : 'https://api.winsloweric.cn/recrearay/api/blacklist.php',
		async : true,
		success : function(callback) {
...
$(document).ajaxError(function (event, xhr, options) {
		mdui.snackbar({
			message: '未知网络IO错误 请反馈给 i@winsloweric.cn',
			position: 'right-top'
		});
